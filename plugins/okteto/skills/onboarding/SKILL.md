...
1. **`docker-compose.yml` / `compose.yaml`** — the richest signal for services and dev. If present, it is the primary blueprint for `build:` and `dev:`. See the mapping table below.
...
**These signals are complementary, not exclusive.** A repo with both a `docker-compose.yml` and a Helm chart is common: compose drives `build:` and `dev:`; the chart drives `deploy:`. Apply each signal to its own section of the manifest rather than treating priority order as a winner-takes-all.
...
| `ports` (Level 2+) | exposed via the `deploy:` artifact (Helm/k8s); also keep `dev.<name>.forward` for active dev sessions |
...
3. **`deploy:` section** (Level 2+) — `helm` or `kubectl` command pointing at the discovered artifact. See the schema example below.
4. **`dev:` section** (always) — one entry per service the user wants in dev mode. Include `image`, `command`, and `sync`.
...
deploy:
  - name: Deploy chart
    command: helm upgrade --install myapp ./chart  # discovered chart at ./chart

dev:
  api:
    image: okteto/golang:1.22  # api/go.mod declares Go 1.22
